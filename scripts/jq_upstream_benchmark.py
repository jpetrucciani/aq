...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark jq upstream direct success cases against jq and aq."
    )
    parser.add_argument(
        "--source-url",
        action="append",
    ...
    parser.add_argument(
        "--json-out",
        default="benchmarks/jq-upstream-benchmark.json",
        help="Path for the JSON report, relative to the repo root.",
    )
    parser.add_argument(
        "--markdown-out",
        default="benchmarks/jq-upstream-benchmark.md",
...
def compare_case(
    ...
    jq_result: dict[str, Any],
    tolerance: float,
) -> dict[str, Any]:
    if aq_result["status"] != "passed" or jq_result["status"] != "passed":
        return {
            "status": "uncomparable",
            "aq_over_jq_ratio": None,
            "faster_tool": None,
        }

    aq_median = aq_result["median_seconds"]
    jq_median = jq_result["median_seconds"]
    if (
        not isinstance(aq_median, float)
        or not isinstance(jq_median, float)
        or jq_median <= 0
    ):
        return {
            "status": "uncomparable",
...
def summarize_cases(cases: list[dict[str, Any]], tolerance: float) -> dict[str, Any]:
    ...
    ratios = [
        case["comparison"]["aq_over_jq_ratio"]
        for case in compared_cases
        if isinstance(case["comparison"]["aq_over_jq_ratio"], float)
    ]
    ...
    jq_faster_cases = [
        case for case in compared_cases if case["comparison"]["faster_tool"] == "jq"
    ]
    roughly_equal_cases = [
        case
        for case in compared_cases
    ...
    heaviest_cases = sorted(
        ...
        + (case["jq"]["median_seconds"] or 0.0),
        reverse=True,
    )
    slowest_relative_cases = sorted(
        jq_faster_cases,
        key=lambda case: case["comparison"]["aq_over_jq_ratio"] or 0.0,
        reverse=True,
    )
    fastest_relative_cases = sorted(
        aq_faster_cases,
        key=lambda case: case["comparison"]["aq_over_jq_ratio"] or float("inf"),
    )

    aq_total = sum(case["aq"]["median_seconds"] or 0.0 for case in compared_cases)
    jq_total = sum(case["jq"]["median_seconds"] or 0.0 for case in compared_cases)

    return {
        "compared_cases": len(compared_cases),
        "aq_faster_cases": len(aq_faster_cases),
        "jq_faster_cases": len(jq_faster_cases),
        "roughly_equal_cases": len(roughly_equal_cases),
        "uncomparable_cases": len(uncomparable_cases),
        "aq_total_median_seconds": aq_total,
        "jq_total_median_seconds": jq_total,
        "median_aq_over_jq_ratio": statistics.median(ratios) if ratios else None,
        "geometric_mean_aq_over_jq_ratio": geometric_mean_ratio(ratios),
...
def render_case_timing_line(case: dict[str, Any]) -> str:
    ...
    jq_median = case["jq"]["median_seconds"]
    ratio = case["comparison"]["aq_over_jq_ratio"]
    return (
        f"`{format_case_label(case)}` `{case['program']}`"
        f", jq `{jq_median * 1000:.2f}ms`, aq `{aq_median * 1000:.2f}ms`, aq/jq `{render_ratio(ratio)}`"
    )
...
def render_failure_line(case: dict[str, Any]) -> str:
    return (
        f"`{format_case_label(case)}` `{case['program']}`"
        f", jq `{case['jq']['status']}`, aq `{case['aq']['status']}`"
    )
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- jq binary: `{report['jq_binary']}`",
        f"- jq version: `{report['jq_version']}`",
        ...
        f"- jq faster cases: `{summary['jq_faster_cases']}`",
        f"- Roughly equal cases: `{summary['roughly_equal_cases']}`",
        f"- Uncomparable cases: `{summary['uncomparable_cases']}`",
        f"- Sum of jq medians: `{summary['jq_total_median_seconds']:.3f}s`",
        f"- Sum of aq medians: `{summary['aq_total_median_seconds']:.3f}s`",
        f"- Median aq/jq ratio: `{render_ratio(summary['median_aq_over_jq_ratio'])}`",
        f"- Geometric mean aq/jq ratio: `{render_ratio(summary['geometric_mean_aq_over_jq_ratio'])}`",
        "",
    ...
    if summary["top_heaviest_cases"]:
        lines.extend(["", "### Heaviest Cases", ""])
        for case in summary["top_heaviest_cases"]:
            lines.append(f"- {render_case_timing_line(case)}")

    if summary["uncomparable_cases"]:
        lines.extend(["", "### Uncomparable Cases", ""])
        for case in [
            case
            for case in report["cases"]
            if case["comparison"]["status"] != "compared"
        ][:20]:
...
def main() -> int:
    ...
    jq_binary = Path(args.jq_binary)
    if not jq_binary.is_absolute():
        jq_binary = (repo_root / jq_binary).resolve() if "/" in args.jq_binary else jq_binary
    ...
    jq_version = read_version([args.jq_binary], repo_root)
    aq_version = read_version([str(aq_binary)], repo_root)

    benchmark_cases: list[dict[str, Any]] = []
    benchmark_start = time.perf_counter()
    with tempfile.TemporaryDirectory(prefix="aq-jq-bench-modules-") as module_dir_raw:
        ...
        for index, case in enumerate(harvested_cases, start=1):
            ...
            jq_result = benchmark_tool(
                tool_name="jq",
                command=[args.jq_binary, "-c", case.program],
                case=case,
            ...
            comparison = compare_case(aq_result, jq_result, args.comparison_tolerance)
    ...
    report = {
        ...
        "measured_runs": args.measured_runs,
        "timeout_seconds": args.timeout_seconds,
        "jq_binary": path_display(jq_binary, repo_root),
